...
if(isset($_GET['action']) && $_GET['action']==1){
    ...
    if(!empty($nomeArquivo)){
      ...
      move_uploaded_file($nomeTemporario, ($arq_new));
		...
		$sel = $db->select("UPDATE sistema SET img='$nomeArquivo', texto='$texto', titulo='$titulo' ");	

	}else {

      $sel = $db->select("UPDATE sistema SET  texto='$texto', titulo='$titulo' ");
...
$sel = $db->select("SELECT * FROM sistema LIMIT 1");
$ln = $db->expand($sel);
?>
...
<div class="panel panel-primary">
	...
	<div class="panel-heading">
		<h4 class="panel-title"><h4>Texto sobre o sistema Concoc</h4></h4>
	</div>
