...
if(isset($_GET['action']) && $_GET['action']==3){
	...
	$sel = $db->select("DELETE FROM cadastros WHERE id='$id' LIMIT 1");
...
<ul class="list-group">
	...
	<?php
		...
		$sel = $db->select("SELECT * FROM cadastros GROUP BY email ORDER BY id DESC LIMIT $inicio,$registros");
		if($db->rows($sel)){
			$x=1;	
				...
				echo'
					...
					<li class="list-group-item">
						...
						<div id="pro'.$x.'" class="collapse panel-collapse">
							<br>
							<p><strong>Nome:</strong> '.$yy['nome'].'</p>			
							<p><strong>Escola:</strong> '.$yy['escola'].'</p>			
							<p><strong>Ano pretendido:</strong> '.$yy['ano'].'</p>			
							<p><strong>E-mail:</strong> '.$yy['email'].'</p>			
							<p><strong>Whats:</strong> '.$yy['whats'].'</p>			
						</div>
